...
class TotalEndpointsCount @Inject() (val integrationRepository: IntegrationRepository) extends MetricSource with Logging {
  ...
  override def metrics(implicit ec: ExecutionContext): Future[Map[String, Int]] = {
    integrationRepository
      ...
      .map { totalEndpointsCount =>
        logger.info(s"[METRIC] Collecting metrics for Total Endpoints Count - $totalEndpointsCount")
        Map("totalEndpointsCount" -> totalEndpointsCount)
      }
  }
