...
import io.swagger.v3.oas.models.media.{Schema => OasSchema, ComposedSchema => OasComposedSchema, ArraySchema => OasArraySchema}
...
trait OASV3SchemaAdapter {
  ...
  private def createDefaultSchema(mayBeSchemaName: Option[String], oasSchemaVal: OasSchema[_]) = {
    ...
    val enums: List[String] = Option(oasSchemaVal.getEnum)
                                .map(enums => 
                                  enums.asScala
                                    .map(Option(_))
                                    .map(_.getOrElse("null").toString) // to handle an enum value that is 'null'. See 'Nullable enums' here: https://swagger.io/docs/specification/data-models/enums/
                                    .toList)
                                .getOrElse(List.empty)
